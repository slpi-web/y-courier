...
echo $form->dropDownListGroup($model, 'client_status', array(
    'widgetOptions' => array(
        'data' => $model->getClientStatusLabels(),
        'htmlOptions' => array(
        ),
    )
));
...
<div id="form-status-synced" style="<?php if ($model->status != CseDelivery::STATUS_SYNCED) echo 'display:none;'; ?>">
    <?php
    echo $form->textFieldGroup($model, 'cse_id', array(
        'label' => $model->getAttributeLabel('cse_id'),
        'labelOptions' => array(
            'required' => true,
        ),
        'widgetOptions' => array(
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                $this->widget('ext.Cse.CseCitySelectorWidget', array(
                    ...
                    'model' => $model,
                    'type' => 'sender',
                    'ajaxCountryInfoUrl' => array('/admin/ajax/getCseCountyInfo'),
                    'ajaxRegionAutocompleteUrl' => array('/admin/ajax/autocompleteCseRegion'),
                    'ajaxAreaAutocompleteUrl' => array('/admin/ajax/autocompleteCseArea'),
                    'ajaxCityAutocompleteUrl' => array('/admin/ajax/autocompleteCseCity'),
                ));
...
<div class="text-center">
    ...
    <?php $this->widget(
        'ext.WLadda.TbLaddaButton',
        array('buttonType' => 'submit', 'label' => Yii::t('view_admin', 'Save and Sync'), 'htmlOptions' => array('id' => 'save_and_sync', 'class' => 'btn-warning', 'name' => 'sync', 'style' => ($model->status == CseDelivery::STATUS_SYNCED ? 'display:none;' : '')))
    ); ?>
</div>
<?php $this->endWidget(); ?>
...
$this->widget('ext.Cse.CseAddressBookWidget', array(
    'saveUrl' => array('/admin/cseDelivery/addressbook', 'action' => 'save'),
    'openUrl' => array('/admin/cseDelivery/addressbook', 'action' => 'open'),
    'userSelect' => true,
    'modelName' => $modelName,
));
...
$userInfoUrl = Yii::app()->createUrl('/admin/ajax/getUserData');
...
$cs->registerScript('status-switches', '
    $("#'.$modelName.'_status").change(function (e) {
        if ($(this).val() == "'.CseDelivery::STATUS_SYNCED.'") {
            $("#form-status-synced").show();
            $("#save_and_sync").hide();
        } else {
            $("#form-status-synced").hide();
            $("#save_and_sync").show();
        }
    });
');
