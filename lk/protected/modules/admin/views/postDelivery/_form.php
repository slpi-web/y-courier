...
$updateUrl = Yii::app()->createUrl('/admin/ajax/getUserData');
$cs->registerScript('update-business-centers', '
    $("#PostDeliveryAdmin_user_id").change(function(e){
        select = $("#PostDeliveryAdmin_business_center_id");
        select.empty();
        $.ajax({
            url: "'.$updateUrl.'",
            data: {
            ...
            success: function(response) {
                if (response.status && response.data) {
                    if (response.data.organization)
                        $("#PostDeliveryAdmin_organization").val(response.data.organization).attr("value", response.data.organization);
