...
class TbButtonColumn extends CButtonColumn {
	...
	protected function renderButton($id, $button, $row, $data) {
		...
		if (isset($button['evaluateOptions']) && is_array($button['evaluateOptions'])) {
			foreach ($button['evaluateOptions'] as $evaluableOption) {
				if (isset($options[$evaluableOption])) {
					$options[$evaluableOption] = $this->evaluateExpression($options[$evaluableOption], array(
						'data' => $data,
					));
				}
			}
