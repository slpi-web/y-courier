...
class TbDateRangePicker extends TbBaseInputWidget {
	...
	/**
	 * @var string|null the initial value
	 */
	...
	public function registerScript($selector, $options = array(), $callback = null) {
		...
		$callback = $callback ? ', ' . CJavaScript::encode($callback) : '';
		ob_start();
		echo "jQuery('{$selector}').daterangepicker({$options}{$callback})";
		foreach ($this->events as $event => $handler) {
			echo ".on('{$event}', " . CJavaScript::encode($handler) . ")";
		}
		if ($this->initValue !== null)
			echo '.val('.CJavaScript::encode($this->initValue).')';
		echo ';';
