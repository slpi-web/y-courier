...
class ArrayValidator extends CValidator {
	...
	public $convertToArray = false;

	public $splitBy = false;
	...
	protected function validateAttribute($object, $attribute)
	{
		if ($this->isEmpty($object->$attribute)) {
			...
			$this->addError($object, $attribute, Yii::t('array_validator', '{attribute} is not allowed to be empty', array('{attribute}'=>$attribute)));
			return;
		}

		if ($this->convertToArray && !is_array($object->$attribute))
			$object->$attribute = array($object->$attribute);

		if (!is_array($object->$attribute) && $this->splitBy) {
			$object->$attribute = explode($this->splitBy, $object->$attribute);
		}

		if (!is_array($object->$attribute)) {
			$this->addError($object, $attribute, Yii::t('array_validator', 'You are trying to validate a non-array attribute'));
			return;
		}
		...
		if ($object->hasErrors($attribute)) {
			...
			$object->addError($attribute, Yii::t('array_validator', 'At least one element has an error:').' '.$firstError);
		}
	}
